...
res_char_to_name = {}
for name, char in list(res_name_to_char.items()):
...
def get_res_chi_topology(res_type):
  ...
  if res_type in ["HSE"]:
    res_type = "HIS"
  if res_type in _chi_topology:
    result = copy.deepcopy(_chi_topology[res_type])
  else:
    result = []
  # deal with some name mangling for ILE: rename CD -> CD1 in the template
  if res_type == "ILE":
...
def calculate_chi(residue, j):
  ...
  raise ValueError("No Chi%d angle for res %d" % (j, residue.num))
...
def ResidueFromType(res_type):
  for res in _template.residues():
    if res_type == res.type:
      return res.copy()
  raise ValueError("Don't have template for %s").with_traceback(res_type)
...
def get_n_chi(res):
  if res.type in _chi_topology:
    return len(_chi_topology[res.type])
  else:
    return 0
...
def mutate(res, res_type):
  new_res = ResidueFromType(res_type)

  old_chi = []
  for j in range(0, get_n_chi(res)):
    try:
      old_chi.append(get_chi(res, j))
    except:
      old_chi.append(0.0)

  delete_atom_list = [atom.type for atom in res.atoms()
                     if not new_res.has_atom(atom.type)]

  for atom_type in delete_atom_list:
    res.erase_atom(atom_type)

  ref1 = res.atom("N").pos
  ref2 = res.atom("CA").pos
  ref3 = res.atom("C").pos
  ...
  for atom in new_res.atoms():
    if atom.type not in backbone_atom_types:
      if res.has_atom(atom.type):
        res.atom(atom.type).pos = atom.pos
      else:
        res.insert_atom(atom)

  res_num = res.num
  res_insert = res.insert
  res.set_num(res_num, res_insert)
  res.type = res_type
  for atom in res.atoms():
    atom.res_type = res_type

  n_chi = min(len(old_chi), get_n_chi(res))
  for j in range(0, n_chi):
    set_chi(res, j, old_chi[j])
...
class Protein(polymer.Polymer):
  ...
  def n_chi(self, i):
    if self.residue(i).type in _chi_topology:
